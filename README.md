...
|---|---|---|---|
| [**Raspberry Pi Pico**](docs/start-rpi_pico.md) (RP2040) | [`provision-rpi_pico.uf2`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-rpi_pico.uf2) | **No** | Hold BOOTSEL, plug in, copy the file onto the drive that appears. Unbrickable — the RP2040 mask ROM always gives you BOOTSEL back |
| [**Raspberry Pi Pico 2 W**](docs/start-rpi_pico2.md) (RP2350) | [`provision-rpi_pico2w.uf2`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-rpi_pico2w.uf2) | **No** | Hold BOOTSEL, plug in, copy the file onto the drive that appears. Unbrickable — the RP2350 boot ROM always gives BOOTSEL back |
| [**Adafruit Feather nRF52840**](docs/start-adafruit_feather_nrf52840.md) (nRF52840) | [`provision-adafruit_feather_nrf52840-mcuboot.hex`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-adafruit_feather_nrf52840-mcuboot.hex) + [`provision-adafruit_feather_nrf52840-slot0.hex`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-adafruit_feather_nrf52840-slot0.hex) | Yes, SWD | SWD, both files in order without resetting in between. **Back the board up first — this erases the Adafruit UF2 bootloader and its UICR settings, and there is no ROM loader to fall back on**  |
| [**Waveshare ESP32-S3 (DevKitC-compatible)**](docs/start-esp32s3_devkitc.md) (ESP32-S3) | [`provision-esp32s3_devkitc-mcuboot.bin`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-esp32s3_devkitc-mcuboot.bin) + [`provision-esp32s3_devkitc-slot0.bin`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-esp32s3_devkitc-slot0.bin) | **No** | esptool over the board's own USB, both binaries plus the identity in one write. Third architecture, and the first Espressif board in Zephyr to declare cdc-acm nodes. The S3 muxes one USB PHY between the ROM's USB-Serial/JTAG and USB-OTG, so the running application owns the connector — which is why re-provisioning needs the BOOT button while a factory-fresh board does not. Requires deferred logging: see the snippet conf in runtt-zephyr-module |
| [**Adafruit RP2040 CAN Bus Feather**](docs/start-adafruit_feather_canbus_rp2040.md) (RP2040 + MCP25625) | [`provision-adafruit_feather_canbus_rp2040.uf2`](https://github.com/shaunmulligan/runtt-boards/releases/latest/download/provision-adafruit_feather_canbus_rp2040.uf2) | **No** | Hold BOOTSEL, plug in, copy the file onto the drive that appears. Unbrickable — the RP2040 mask ROM always gives you BOOTSEL back. The first board with both transports: dual-CDC USB and SMP over ISO-TP on CAN (500 kbit/s, node id from the identity record) |
