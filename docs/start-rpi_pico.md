...
./scripts/runtt-board provision rpi_pico --name pico-01
```

Hold **BOOTSEL** while plugging the board in when asked. The name becomes the
board's USB serial, so `usb:pico-01` addresses it from now on. Success looks
like: `flashed: the board left BOOTSEL.`
...
```bash
podman build --build-arg BOARD=rpi_pico/rp2040/mcuboot -t my-app:v1 .
```
...
podman run --rm --network none --runtime=/usr/local/bin/runtt \
  --annotation dev.runtt.target=usb:pico-01 my-app:v1
```
